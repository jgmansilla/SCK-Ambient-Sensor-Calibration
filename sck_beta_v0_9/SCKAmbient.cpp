...
///////////////// CO Sensor Calibration ///////////////// 
  ...
  float RsCO;                          ////  RsCO value
  float RsNO2;                         ////  RsNO2 value
  float RsCOTemp;                      ////  Rs value with temperature correction
  float x;                             ////  RsCO/R0CO Ratio
  float y;                             ////  RsNO2/R0NO2 Ratio
  float COConcentration;               ////  CO concentration in ppm
  float COConcentrationTempDatasheet;  ////  CO concentration in ppm with Temperature correction (as per datasheet)
  float tempCorrectionFactor;          ////  Temperature correction
  float NO2Concentration;              ////  NO2 concentration in ppb
...
///////////////// CO Sensor Calibration ///////////////// 
    ...
    Serial.println(" ==========  CO Sensor Calibration  ========== ");
    Serial.println("Sensor calibrated for temperatures between 10 a 70 Centigrade degrees and ");
    Serial.println("Relative humidity from 5% to 95% RH as per datasheet ");

    temperature=(float)(value[0])/1000;                                         //// Conversion to ºC
    humidity=(float)(value[1])/1000;                                            //// Conversion to %RH

    Serial.print("Relative Humidity: ");
    Serial.print(humidity);                                                     //// Relative Humidity in %RH
    Serial.println(" %RH");  

    Serial.print("Temperature: ");
    Serial.print(temperature);                                                  //// Temperature in ºC
    Serial.println(" C");  

    RsCO=(float)(value[5]);                                                     //// RsCO in Ohms
    x=RsCO/RoCO;                                                                //// Rs/Ro Ratio calculation
    COConcentration=-75.6518+exp(-143.735*x)+41.1398/x+27.3354*x;               //// Concentration Function (ppm)
    Serial.print("CO Concentration in ppm without temperature correction: ");   //// Concentration in ppm
    Serial.print(COConcentration);
    Serial.println(" ppm");

    tempCorrectionFactor=250/(3313.44/temperature+194.11-2.51*temperature);     //// Temperature Correction Function,
    RsCOTemp=tempCorrectionFactor*RsCO;                                         //// Applying correction
    x=RsCOTemp/RoCO;
    COConcentrationTempDatasheet=-75.6518+exp(-143.735*x)+41.1398/x+27.3354*x;  //// Concentration Function (ppm)
    Serial.print("CO Concentration in ppm with temperature correction: ");      //// Concentration in ppm
    Serial.print(COConcentrationTempDatasheet);
    Serial.println(" ppm");

    RsNO2=(float)(value[6]);                                                     //// RsNO2 in Ohms
    y=RsNO2/RoNO2;                                                               //// Rs/Ro Ratio calculation
    NO2Concentration=0.0473538*y-60.4778+50.8654*pow(y,0.327898);                //// Concentration Function (ppb)
    Serial.print("NO2 Concentration in ppb without temperature correction: ");   //// Concentration in ppb
